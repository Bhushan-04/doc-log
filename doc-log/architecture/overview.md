...
`createOpenWikiContentSnapshot` SHA256-hashes all files under `doc-log/` except `.last-update.json`. Metadata is written only when the hash differs after a successful init/update run — avoiding timestamp bumps when the agent made no doc edits.
...
| Symbol                      | Value / role                                    |
| --------------------------- | ----------------------------------------------- |
| `OPEN_WIKI_DIR`             | `"doc-log"` (output directory in target repos)  |
