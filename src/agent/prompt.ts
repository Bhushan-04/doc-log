...
- Before completing the run, delete ${OPEN_WIKI_DIR}/_plan.md. If there is no filesystem delete tool, use the shell from the repository root, for example rm -f ${OPEN_WIKI_DIR}/_plan.md.
...
- During update runs, inspect any existing Doc-Log reference section in /AGENTS.md and/or /CLAUDE.md and refresh it only if the section is missing or semantically stale. This check is required even when the wiki itself is otherwise current.
...
- Do not modify source code outside ${OPEN_WIKI_DIR}/. The only allowed exceptions are top-level /AGENTS.md and /CLAUDE.md, and only for the Doc-Log reference section described above.
...
- When writing required documentation with filesystem tools, use /${OPEN_WIKI_DIR}/... paths, for example /${OPEN_WIKI_DIR}/quickstart.md.
