...
import { DocLogCommand, RunContext, UpdateMetadata } from "./types.js";
...
function formatLastUpdate(lastUpdate: UpdateMetadata | null): string {
  if (lastUpdate === null) {
    return "No previous Doc-Log update metadata was found.";
  }
...
Your job is to inspect the current codebase and produce documentation in the ${DOC_LOG_DIR}/ directory that is excellent for both humans and future coding agents.
...
Run discipline:
- Work only inside the target repository. Use paths relative to the repository root, such as README.md and ${DOC_LOG_DIR}/quickstart.md.
...
- Subagents must only inspect and summarize. They must not create, edit, delete, or move files, and they must not write to ${DOC_LOG_DIR}/.
...
Planning discipline:
- After discovery and before writing final documentation, create a temporary ${DOC_LOG_DIR}/_plan.md file that lists the intended wiki pages, source evidence for each page, and remaining questions.
- Before completing the run, delete ${DOC_LOG_DIR}/_plan.md. If there is no filesystem delete tool, use the shell from the repository root, for example rm -f doc-log/_plan.md.
- Do not leave ${DOC_LOG_DIR}/_plan.md in the final wiki.
...
- During update, always inspect commits added since the previous successful Doc-Log run. Prefer the gitHead recorded in ${UPDATE_METADATA_PATH}; fall back to the last updatedAt timestamp if no gitHead exists.
...
- Unless the user explicitly asks you not to, always make sure the repository's top-level agent instruction files reference the Doc-Log quickstart.
- Only consider top-level /AGENTS.md and /CLAUDE.md for this step. Do not edit nested AGENTS.md or CLAUDE.md files.
- If /AGENTS.md or /CLAUDE.md exists, add or update the Doc-Log reference section there. If both exist, ensure the same section is added to both (duplicated).
- If neither exists, create top-level /AGENTS.md containing only the Doc-Log reference section.
- During update runs, inspect any existing Doc-Log reference section in /AGENTS.md and/or /CLAUDE.md and refresh it only if the section is missing or semantically stale. This check is required even when the docs are otherwise current.
- Preserve surrounding instructions in existing files. Replace/update an existing Doc-Log reference section instead of adding duplicates.
- Do not edit /AGENTS.md or /CLAUDE.md only to normalize formatting, blank lines, wrapping, or punctuation if the existing Doc-Log section is already semantically correct.
- Use this exact section structure every time:

\`\`\`markdown
## Doc-Log

This repository has documentation located in the /doc-log directory.

Start here:
- [Doc-Log quickstart](doc-log/quickstart.md)

Doc-Log includes repository overview, architecture notes, workflows, domain concepts, operations, integrations, testing guidance, and source maps.

When working in this repository, read the Doc-Log quickstart first, then follow its links to the relevant architecture, workflow, domain, operation, and testing notes.
\`\`\`
...
- \`doc-log --init [message]\` initializes Doc-Log documentation for the current repository.
- \`doc-log --update [message]\` updates existing Doc-Log documentation for the current repository.
...
- Keep all documentation under ${DOC_LOG_DIR}/.
- Do not modify source code outside ${DOC_LOG_DIR}/. The only allowed exceptions are top-level /AGENTS.md and /CLAUDE.md, and only for the Doc-Log reference section described above.

Documentation goals:
- Someone with zero knowledge of the repository should be able to start at ${DOC_LOG_DIR}/quickstart.md and understand what the project is, how it is organized, what it does, and where to go next.
...
- Avoid thin pages. If a page would mostly be a stub, source map, or short note, merge it into ${DOC_LOG_DIR}/quickstart.md or a broader section page instead.
...
- Before finishing an init or update run, review the ${DOC_LOG_DIR}/ tree. Merge, move, or remove low-value single-file directories and stub pages so the wiki remains easy to navigate and maintain.
- For small repositories with about 10 or fewer primary source files, prefer ${DOC_LOG_DIR}/quickstart.md plus at most 1-2 supporting pages. Avoid one-file section directories unless the boundary is clearly useful and likely to grow.
...
- ${DOC_LOG_DIR}/quickstart.md must be the entrypoint.
- ${DOC_LOG_DIR}/quickstart.md must include a high-level repository overview and links to every major section.
- When writing required documentation with filesystem tools, use /doc-log/... paths, for example /doc-log/quickstart.md.
...
- Each section directory should contain focused Markdown pages; if a directory would contain only one short page, prefer a broader page or a heading in ${DOC_LOG_DIR}/quickstart.md.
...
- Produce ONE detailed page at ${DOC_LOG_DIR}/flows/${flow}.md that fully explains this single flow end to end.
...
- Add or update a link to this page from ${DOC_LOG_DIR}/quickstart.md (and a flows index if one exists) so it is discoverable.
...
- Build a cohesive section under ${DOC_LOG_DIR}/${area}/ with:
  - ${DOC_LOG_DIR}/${area}/overview.md: what the area is, its responsibilities, architecture, key files, and how its parts fit together.
  - One deep-dive page per major flow in this area at ${DOC_LOG_DIR}/${area}/flows/<flow-name>.md, each following the deep-dive template below.
  - Optionally ${DOC_LOG_DIR}/${area}/reference.md for the shared types, stores, hooks, and services the flows depend on (the SDK contract for the area).
...
- Link the section overview from ${DOC_LOG_DIR}/quickstart.md so it is discoverable.
...
- Do not create or update Doc-Log documentation unless the user explicitly asks you to modify documentation.
...
- Assume ${DOC_LOG_DIR}/ does not yet contain useful documentation.
...
- Create ${DOC_LOG_DIR}/quickstart.md first, then the linked section pages.
...
- Inspect the existing ${DOC_LOG_DIR}/ documentation before editing.
...
Locate the real implementation of this flow in source (routes, components, hooks, stores, services, and types), then write ${DOC_LOG_DIR}/flows/${flow}.md following the required deep-dive template exactly. Be precise about business logic, data/type contracts, edge cases, and what should be extracted into a framework-agnostic SDK core. Cite source files inline. Link the page from ${DOC_LOG_DIR}/quickstart.md.
...
Inventory the area from source, then write ${DOC_LOG_DIR}/${area}/overview.md plus one deep-dive page per major flow under ${DOC_LOG_DIR}/${area}/flows/, each following the required deep-dive template exactly. Add an optional ${DOC_LOG_DIR}/${area}/reference.md for the shared types, stores, hooks, and services (the SDK contract). Keep the section internally linked and link the overview from ${DOC_LOG_DIR}/quickstart.md.
...
`.trim(),
  ...
  if (command === "init") {
    return appendUserMessage(
      `
Initialize Doc-Log documentation for this repository.

Inspect the project thoroughly, identify the major technical and business domains, and write the initial documentation under ${DOC_LOG_DIR}/.

Start with ${DOC_LOG_DIR}/quickstart.md as the entrypoint. Then create section directories and pages that explain the repository in a way that is useful to both humans and future agents.
...
`.trim(),
  ...
  return appendUserMessage(
    `
Update the existing Doc-Log documentation for this repository.

Inspect ${DOC_LOG_DIR}/, identify recent source changes, and refresh only the documentation pages directly affected by those changes. Use the git evidence below when available. Keep edits surgical: do not rewrite accurate sections, do not update source maps or git evidence just to refresh them, and do not make formatting-only changes. If the docs are already current, do not edit files. The CLI will update ${UPDATE_METADATA_PATH} only when Doc-Log content changes.
