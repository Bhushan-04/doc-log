...
function App({ command }: AppProps) {
  ...
  useEffect(() => {
    ...
    if (!process.env[apiKeyEnvKey] && !process.stdin.isTTY) {
      setRunState({
        status: "error",
        message: `${apiKeyEnvKey} is required. Run doc-log in an interactive terminal to save credentials.`,
      });
      return;
    }
  ...
  if (runState.status === "init-setup-saved") {
    return (
      <Box flexDirection="column">
        ...
        />
        {runState.result.savedApiKey || runState.result.savedModelId ? (
        ...
        <StatusLine tone="active" label="Next" value="starting doc-log" />
      </Box>
    );
  }
...
function ErrorDiagnosticsPanel({
  ...
  return (
    <Panel title="Error Diagnostics">
      <Text color="gray">
        DOC_LOG_DEBUG=1 is enabled. Only allowlisted, non-secret error fields
        are shown.
      </Text>
      {diagnostics.map((diagnostic) => (
...
function RunLogLine({
  ...
  const contentWidth = getLogTextWidth();
  ...
  return (
    <Box flexDirection="row" marginBottom={1} width="100%">
      ...
      <Box flexGrow={1} width={contentWidth}>
        {streaming ? (
          <Text wrap="wrap">{item.content.trim()}</Text>
        ) : (
          <MarkdownText markdown={item.content.trim()} width={contentWidth} />
        )}
      </Box>
    </Box>
  );
...
function getLogTextWidth(): number {
  const columns = process.stdout.columns;

  if (typeof columns === "number" && columns > 0) {
    return Math.max(40, columns - 6);
  }
...
function ChatInput({
  ...
  async function saveProviderSelection(rawProvider: string) {
    ...
    if (provider === null) {
      setError("Enter a valid provider: cursor.");
      return;
    }
...
function isDebugMode(): boolean {
  return process.env.DOC_LOG_DEBUG === "1";
}

function shouldShowCredentialDiagnostics(): boolean {
  return isDebugMode() || process.env.DOC_LOG_DEBUG_CREDENTIALS === "1";
}
...
function getErrorMessage(error: unknown): string {
  const message =
    error instanceof Error ? error.message : "Doc-Log agent run failed.";
...
function sanitizeDiagnosticText(value: string): string {
  let sanitized = value;
  const secret = process.env[CURSOR_API_KEY_ENV_KEY];

  if (secret && secret.length > 0) {
    sanitized = sanitized
      .split(secret)
      .join(`[REDACTED:${CURSOR_API_KEY_ENV_KEY}]`);
  }
  ...
  return sanitized
    ...
    .replace(/\bkey_[A-Za-z0-9_-]+/gu, "[REDACTED:API_KEY]")
...
function resolveStartupCommand(command: AppCommand): AppCommand {
  if (
    command.kind === "run" &&
    !command.dryRun &&
    command.shouldStart &&
    ...
    if (!hasProviderKey) {
      return {
        kind: "error",
        exitCode: 1,
        message: `${apiKeyEnvKey} is required for non-interactive runs. Run doc-log in an interactive terminal to save credentials.`,
      };
    }
  }
